...
int SGX_CDECL main(int argc, char* argv[]) {
  ...
  size_t output_array_size =
      number_of_items * sizeof(prochlo::shuffler::AnalyzerItem);
  ...
  int file_descriptor_output =
      open(output_filename, O_RDWR | O_CREAT | O_TRUNC, (mode_t)0600);
  if (file_descriptor_output == -1) {
    perror("Error opening output file for writing");
    exit(EXIT_FAILURE);
  }
  seek_result = lseek(file_descriptor_output, output_array_size - 1, SEEK_SET);
  if (seek_result == -1) {
    close(file_descriptor_output);
    perror("Error calling lseek() to 'stretch' the output file");
    exit(EXIT_FAILURE);
  }
  write_result = write(file_descriptor_output, "", 1);
  if (write_result != 1) {
    close(file_descriptor_output);
    perror("Error writing last byte of the output file");
    exit(EXIT_FAILURE);
  }
  map = mmap(0, output_array_size, PROT_READ | PROT_WRITE, MAP_SHARED,
             file_descriptor_output, 0);
  if (map == MAP_FAILED) {
    perror("Error mmapping the output array");
    exit(EXIT_FAILURE);
  }
  AnalyzerItem* decrypted_items = reinterpret_cast<AnalyzerItem*>(map);

  // IntermediateShufflerItems
  int file_descriptor_intermediate = open(intermediate_filename, O_RDONLY);
  if (file_descriptor_intermediate == -1) {
    perror("Error opening intermediate file for reading");
    exit(EXIT_FAILURE);
  }

  if (fstat(file_descriptor_intermediate, &stat_buffer) ==
      -1) { /* To obtain file size */
    perror("Couldn't fstat intermediate file.");
    exit(EXIT_FAILURE);
  }
  if ((size_t)stat_buffer.st_size < intermediate_array_size) {
    fprintf(stderr,
            "Intermediate file too small (%ld) for the number of items (%ld). "
            "Should "
            "be %ld.\n",
            stat_buffer.st_size, number_of_intermediate_items,
            intermediate_array_size);
    exit(EXIT_FAILURE);
  }
  map = mmap(0, intermediate_array_size, PROT_READ, MAP_SHARED,
             file_descriptor_intermediate, 0);
  if (map == MAP_FAILED) {
    perror("Error mmapping the intermediate array");
    exit(EXIT_FAILURE);
  }
  int madvise_result = madvise(map, intermediate_array_size, MADV_SEQUENTIAL);
  if (madvise_result == -1) {
    perror("madvise MADV_SEQUENTIAL failed");
    exit(EXIT_FAILURE);
  }
  madvise_result = madvise(map, intermediate_array_size, MADV_DONTDUMP);
  if (madvise_result == -1) {
  ...
  printf(
      "Cleaning up with data size=%lu, crowd ID size=%lu, ShufflerItem "
  ...
  sgx_status_t result =
      ecall_clean_up(global_eid, number_of_items, number_of_buckets, chunk_size,
                     stash_chunks, clean_up_window, (void*)decrypted_items,
                     (void*)encrypted_intermediate_items, symmetric_key);
  if (result != SGX_SUCCESS) {
    printf("ecall_clean_up failed with error code (%i)\n", result);
  ...
  if (munmap(decrypted_items, output_array_size) == -1) {
